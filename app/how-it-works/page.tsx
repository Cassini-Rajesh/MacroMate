    ...
    <div className="min-h-screen bg-white">
      ...
      <nav className="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-md border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            ...
            <Link href="/how-it-works" className="text-green-500 font-medium hover:text-green-600 transition-colors px-3 py-2">
              How It Works
            </Link>
      ...
      <main className="pt-32 pb-20 px-4">
        <div className="max-w-6xl mx-auto">
          <h1 className="text-5xl font-black text-gray-900 text-center mb-16">
          ...
          <div className="mb-20">
            <div className="grid md:grid-cols-3 gap-8">
              {/* Step 1 */}
              <div className={`text-center p-6 rounded-2xl border-2 ${step === 1 ? 'border-green-500 bg-green-50' : 'border-gray-200'}`}>
                <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
                  <Camera className="w-8 h-8 text-green-600" />
                </div>
                <h3 className="text-xl font-bold text-gray-900 mb-2">Snap a Photo</h3>
                <p className="text-gray-600 mb-4">Upload an image of your meal</p>
                {step === 1 && (
              ...
              {/* Step 2 */}
              <div className={`text-center p-6 rounded-2xl border-2 ${step === 2 ? 'border-orange-500 bg-orange-50' : 'border-gray-200'}`}>
                <div className="w-16 h-16 bg-orange-100 rounded-full flex items-center justify-center mx-auto mb-4">
                  {loading ? <Loader2 className="w-8 h-8 text-orange-600 animate-spin" /> : <span className="text-orange-600 font-bold text-2xl">AI</span>}
                </div>
                <h3 className="text-xl font-bold text-gray-900 mb-2">AI Analyzes Your Meal</h3>
              ...
              {/* Step 3 */}
              <div className={`text-center p-6 rounded-2xl border-2 ${step === 3 ? 'border-green-500 bg-green-50' : 'border-gray-200'}`}>
                <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
                  <span className="text-green-600 font-bold text-2xl">✓</span>
                </div>
                <h3 className="text-xl font-bold text-gray-900 mb-2">Get Your Macros</h3>
                <p className="text-gray-600 mb-4">Instant macro breakdown</p>
                {step === 3 && (
                  <div className="bg-white border border-gray-200 rounded-lg p-4 text-left">
                    <h4 className="font-bold text-gray-900 mb-2">Grilled Chicken & Rice Bowl</h4>
                    <div className="space-y-1 text-sm">
                      ...
                      <div className="flex justify-between">
                        <span>Protein:</span>
                        <span className="font-semibold text-green-600">48g</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Carbs:</span>
                        <span className="font-semibold text-orange-600">65g</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Fat:</span>
                        <span className="font-semibold text-purple-600">12g</span>
                      </div>
                    </div>
                  </div>
                )}
...
function ComparisonCard({ title, description, color }: { title: string; description: string; color: string }) {
  const bgColor = color === 'green' ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'
  const titleColor = color === 'green' ? 'text-green-600' : 'text-gray-900'
